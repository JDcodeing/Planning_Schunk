...
bool FMGPlanner::checkSegment(const Eigen::Vector3d& con, const Eigen::Vector3d& point)
{
  ...
  for(size_t i =0; i < obs_num; i++)
  {
    ...
    //  std::cout << "closest point " << closestpoint << std::endl;
      ...
      double closest_dis2 = (sphere_centers[i] - closestpoint).norm();
    //  std::cout<<"&&&&&&&&&&&&&&&&&&& closest_dis:"<<closest_dis<<" "<<closest_dis2*closest_dis2<<std::endl;
...
bool FMGPlanner::smooth_valid(const std::vector<double> v1, const std::vector<double> &v2)
{
	double max_diff=0;
	if(v1.size() != v2.size())
	{
...
bool FMGPlanner::smooth_traj()
{
	...
	{	
		std::cout << "the "<< i <<" th smooth check: "<<std::endl;
		...
		if(smooth_valid(initpTraj[i], last_values))
		{
		...
		else
		{
			...
			std::vector<double> joint_values;
				...
				//robot_state_.setToRandomPositions();
			  	...
			  	while(trynum-->0)
			  	{
			  		if(checkCartPoseCollision(ee_pose.translation(), moveit_init,moveit_res))
					{
						moveit::core::robotStateMsgToRobotState(moveit_res, robot_state_);
			           	...
			           	if(smooth_valid(joint_values, last_values))
			           	{
			           		recomputevalid = true;
				...
				if(!recomputevalid)
				{
						...
						robot_state_.setJointGroupPositions(joint_model_group_, initpTraj[i]);
						visual_tools_->publishRobotState(robot_state_, rviz_visual_tools::BLUE);

							ros::Duration(3).sleep();

		  			robot_state_.setJointGroupPositions(joint_model_group_, last_values);	
		  			visual_tools_2->publishRobotState(robot_state_, rviz_visual_tools::RED);
...
bool FMGPlanner::regenerateIK(const Eigen::Affine3d endpose,  std::vector<double> &joint_values)
{
	...
	geometry_msgs::Pose pose_msg;// = dr::toRosPose(endpose);

	pose_msg.position    = dr::toRosPoint(endpose.translation());
	pose_msg.orientation = dr::toRosQuaternion(Eigen::Quaterniond(endpose.rotation()));
...
bool FMGPlanner::checkCartPoseCollision(const Eigen::Vector3d &position, moveit_msgs::RobotState & init_state_msgs, moveit_msgs::RobotState &res_state_msgs)
{
  	...
  	pose_msg.position = dr::toRosPoint(position);
...
bool FMGPlanner::get_dynamic_midpoints(const Eigen::Vector3d start)
{
...
void FMGPlanner::findCartesianPath()
{
	...
	if(! get_dynamic_midpoints(start)) //get Traj_mid_pos 
	{
...
void FMGPlanner::findCartesianPath_my()
{
	std::cout << "test_dynamic : the start is"<<std::endl;
	std::cout << start<<std::endl;
	if(Traj_mid_pos.size()>0) Traj_mid_pos.clear();
  ...
  for(size_t i = 0; i < traj_mid_size; i++)
	...
	if(GetMidIKSolution(waypoints_pos)) // get interpTraj
		...
		if(checkValidTraj())
			...
			{
				ROS_ERROR_STREAM("smooth failed");
			}
		}
